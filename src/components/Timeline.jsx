    ...
    <section className="relative flex flex-col w-full max-w-xl mx-auto timeline pt-4 md:pt-0">
      {
        user.map((timeline)=> (
          <div key={timeline._id}>
            <DateEntry title={timeline.company_name + " -> " + timeline.jobTitle} />
            <Notes>
              
              <p className="p-3 font-semibold text-slate-800 mb-4">{timeline.summary}</p>
            </Notes>
            {
              timeline.bulletPoints.map((bulletPoints)=> (
                <div key={''}>
                  <Notes>
                  <TimelineEntry title={bulletPoints} timestamp={timeline.startDate} Icon={Briefcase} tint="indigo"></TimelineEntry>
                  </Notes>
                </div>
              ))
            }
